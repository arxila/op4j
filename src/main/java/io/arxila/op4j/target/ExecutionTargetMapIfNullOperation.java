 ...
 *
 */
final class ExecutionTargetMapIfNullOperation implements ExecutionTargetOperation {
    ...
    public ExecutionTargetMapIfNullOperation(final boolean desiredResult, final Structure structure, final IFunction<?,?> executable, final IFunction<?,?> elseExecutable, final Class<?> arrayComponentClass) {
        super();
            ...
            switch(this.structure) {
                ...
                case ARRAY:
                    ...
                    for (int i = 0, z = arrayTarget.length; i < z; i++) {
                        arrayResult[i] =
                            ((arrayTarget[i] == null) != this.desiredResult?
                                (this.elseExecutable == null?
                                        arrayTarget[i] :
                                        this.elseExecutable.execute(arrayTarget[i], new ExecCtxImpl(Integer.valueOf(i)))) :
                ...
                case LIST:
                    ...
                    for (final Object element : listTarget) {
                        listResult.add(
                                ((element == null) != this.desiredResult?
                                        (this.elseExecutable == null?
                                                element :
                                                this.elseExecutable.execute(element, new ExecCtxImpl(Integer.valueOf(iList)))) :
                    ...
                    for (final Object element : setTarget) {
                        setResult.add(
                                ((element == null) != this.desiredResult?
                                        (this.elseExecutable == null?
                                                element :
                                                this.elseExecutable.execute(element, new ExecCtxImpl(Integer.valueOf(iSet)))) :
